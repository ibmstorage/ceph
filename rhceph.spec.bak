...
%global commit e4cafe82fd26577be2b327c948130a6509c721af
...
# number.
%global _smp_ncpus_max %{smp_limit_mem_per_job 3200000}
%endif
%endif
...
Source1:	%{name}-%{version}-%{commit}-changes.tar.gz
Source2:	redhat_dashboard_frontend-%{version}-%{commit}.tar.gz
Source3:	ibm_dashboard_frontend-%{version}-%{commit}.tar.gz
Source4:        blake3-1.5.1-92e4cd71be48fdf9a79e88ef37b8f415ec5ac210.tar.bz2
Source5:        xxHash-0.8.2-bbb27a5efb85b92a0486cf361a8635715a53f6ba.tar.bz2
Source6:	s3select-0a0f6d4.patch
%if 0%{?suse_version}
...
BuildRequires:	python%{python3_pkgversion}-zstandard
BuildRequires:	python%{python3_pkgversion}-pyOpenSSL
%endif
%if 0%{?suse_version}
BuildRequires:	golang-github-prometheus-prometheus
...
to NVMeoF Gateway and provides beacons to the monitor daemo
...
Summary:	Daemon for the libcephfs proxy
%if 0%{?suse_version}
Group:		System/Filesystems
%endif
Requires:	libcephfs2 = %{_epoch_prefix}%{version}-%{release}
...
%prep
%autosetup -p1
# Apply the rdopkg-tar patches
%autosetup -T -D -b 1
# Remove the old dashboard dist
rm -rf src/pybind/mgr/dashboard/frontend/dist
# Apply new dashboard dists
%autosetup -T -D -a 2
%autosetup -T -D -a 3
%autosetup -T -D -a 4 # BLAKE3
%autosetup -T -D -a 5 # xxHash
# submodule patches must be handled separately
# https://github.com/redhat-openstack/rdopkg/issues/16
pushd src/s3select
